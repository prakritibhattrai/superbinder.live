...
// Break the circular dependency by using a separate function
function getDocumentsFromHistory() {
  try {
    const history = gatherLocalHistory();
    return (history.documents || []);
  } catch (error) {
    console.error('Error getting documents from history:', error);
    return [];
  }
}
...
export function useDocuments() {
  ...
  function loadDocuments() {
    try {
      const docsFromHistory = getDocumentsFromHistory();
      if (docsFromHistory.length > 0) {
        documents.value = docsFromHistory;
